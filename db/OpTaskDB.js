...
/**
 * Retrieves a page of projects for a given user ID.
 *
...
opDB.getPageProjects = async function (userId, page) {
  ...
  try {
    ...
    // Connect to the OpTask DB
    const db = client.db(DB_NAME);
    const projectsCollection = db.collection("projects");

    // Retrieve projects for the given user ID, sort them in descending order by _id,
    // and paginate the results based on the page number
    const results = await projectsCollection
      .find({ ownerId: userId })
      .sort({ _id: -1 })
